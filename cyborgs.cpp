...
//  Project 7
//
//  Created by nitya khanna on 27/05/22.
//
...
string Player::move(int dir)
{
    ...
    {
        if (dir==NORTH && m_arena->numberOfCyborgsAt(m_row, m_col)==0)
        {
            return "Player moved north.";
        }
        else if (dir==SOUTH && m_arena->numberOfCyborgsAt(m_row, m_col)==0)
        {
            return "Player moved south.";
        }
        else if (dir == EAST && m_arena->numberOfCyborgsAt(m_row, m_col)==0)
        {
            return "Player moved east.";
        }
        else if (dir == WEST && m_arena->numberOfCyborgsAt(m_row, m_col)==0)
        {
            return "Player moved west.";
        }
        //if player moves into cyborg, set player to dead
    ...
    else if (!attemptMove(*m_arena, dir, m_row, m_col))
    {
        return "Player couldn't move; player stands.";
    }
    return "input error: direction invalid"; //this will never get executed as the takePlayerTurn function won't accept any invalid direction
}
...
Arena::~Arena()
//deleting all cyborgs and the player
{
...
int Arena::numberOfCyborgsAt(int r, int c) const
{
    ...
    //checks if each cyborg's row and col are equal to r and c
    for (int i = 0; i < m_nCyborgs; i++)
    {
...
void Arena::display(string msg) const
{
    ...
    //loop through the displayGrid
    for (r = 1; r <= rows(); r++)
    {
        for (c = 1; c <= cols(); c++)
        {
            //if there is a Cyborg at a certain position
            if (numberOfCyborgsAt(r, c) > 0)
            {
                //loop through Cyborg array to find which cyborg is at that position
                for (int i = 0; i < m_nCyborgs; i++)
                {
                    if (m_cyborgs[i]->row() == r && m_cyborgs[i]->col() == c )
                    {
                        //assign the displayGrid element with the string version of the channel number of the cyborg
                        int ascii0 = 48; //ascii value of 0 is 48
                        int x = m_cyborgs[i]->channel();
                        displayGrid[r-1][c-1] = x + ascii0;
                        break;
                    }
                }
            }
...
string Arena::moveCyborgs(int channel, int dir)
{
    ...
    for (int i = 0 ; i < m_nCyborgs; i++)
    {
        ...
        else if (!willRespond || m_cyborgs[i]->channel()!=channel)
        {
            m_cyborgs[i]->move();
        }
        
        //set player to dead if any cyborg moves into the player
        if (player()->row()== m_cyborgs[i]->row()  && player()->col()== m_cyborgs[i]->col())
        {
            player()->setDead();
        }
        
        //delete cyborg if it is dead
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    ...
    for (int i = 0 ; i < 5 ; i++)
    {
        ...
        total += a.numberOfCyborgsAt(tempR-1, tempC);
        total += a.numberOfCyborgsAt(tempR+1, tempC);
        total += a.numberOfCyborgsAt(tempR, tempC+1);
        total += a.numberOfCyborgsAt(tempR, tempC-1);
        if (a.numberOfCyborgsAt(tempR, tempC)==0
            && (tempR >= 1 && tempR <= a.rows() && tempC >= 1 && tempC <= a.cols())
            && !a.hasWallAt(tempR, tempC))
        {
            numCyb[i] = total;
        }
        else
            numCyb[i]=MAXCYBORGS+1;
    }
    ...
    if (min == numCyb[0])
        return false;
    else if (min == numCyb[1])
    {
        bestDir = NORTH;
        return true;
    }
    else if (min == numCyb[2])
    {
        bestDir = EAST;
        return true;
    }
    else if (min == numCyb[3])
    {
        bestDir = SOUTH;
        return true;
    }
    else if (min == numCyb[4])
    {
        bestDir = WEST;
        return true;
    }
